                         ...
                         if ($k >= 1) {
                            ...
                            //   }
                             ...
                             if($exist){
                              ...
                              if(!db('admin')->where('worknum', $data2['worknum'])->find()) {
                              	db("admin")->insert($data2);
                              	$data3['uid'] = db('admin')->where('worknum', $data2['worknum'])->value('id');
                              	db('auth_group_access')->insert($data3);
                              } else {
                              	db('admin')->where('worknum', $data2['worknum'])->update($data2);
                              	$data3['uid'] = db('admin')->where('worknum', $data2['worknum'])->value('id');
                              	db('auth_group_access')->where('uid', $data2['worknum'])->update($data3);
                              }
...
//dump($admins);die;
	...
	public
		...
		if ( request()->isPost() ) {
			...
			$validate = \think\ Loader::validate( 'Admin' );//dump($validate);die;
			 if ( !$validate->scene( 'edit' )->check( $datas ) ) {				
			 	$this->error( $validate->getError());
			 }
			$data['group_id']=db('auth_group_access')->where(array('uid'=>$data['id']))->value('group_id');
			$data['password'] = $datas['password'];//dump($admins);die;
			$admin = new AdminModel();
			$savenum = $admin->saveadmin( $data, $admins );
			//dump($savenum);die;
			if ( $savenum == '2' ) {
