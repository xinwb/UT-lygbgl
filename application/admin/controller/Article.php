...
class Article extends Common {
	...
	public
	function lst() {
		$artres = db( 'admin' )->field( 'a.worknum,a.name,a.type,a.cateid,b.id,b.sort,b.job,b.birthday,c.catename ' )->alias( 'a' )->join( 'gb_article b', 'a.worknum=b.adminid' )->join( 'gb_cate c', 'c.id=a.cateid' )->where('a.type ="中层干部" or a.type = "科级干部"')->order( 'b.sort,a.cateid,a.worknum' )->paginate( 50 );
		...
		if ( request()->isPost() ) {
			...
			if ( $data[ "part" ] == 0 && $data[ "information" ] == "" ) {
				$artres = db( 'admin' )->field( 'a.worknum,a.name,a.type,a.cateid,b.birthday,b.job,b.sort,b.id,c.catename ' )->alias( 'a' )->join( 'gb_article b', 'a.worknum=b.adminid' )->join( 'gb_cate c', 'c.id=a.cateid' )->where('a.type',"中层干部")->order( 'b.sort,a.cateid,a.worknum' )->paginate( 50 );
			} 
			elseif ( $data[ "part" ] == 0 ) {
				if ( $data[ "selectway" ] == "name" ) {
					$artres = db( 'admin' )->field( 'a.worknum,a.name,a.type,a.cateid,b.birthday,b.job,b.sort,b.id,c.catename ' )->alias( 'a' )->join( 'gb_article b', 'a.worknum=b.adminid' )->join( 'gb_cate c', 'c.id=a.cateid' )->where('a.type',"中层干部")->order( 'b.sort,a.cateid,a.worknum' )->where( 'a.name', $data[ "information" ] )->paginate( 50 );
				} 
				elseif ( $data[ "selectway" ] == "worknum" ) {
					$artres = db( 'admin' )->field( 'a.worknum,a.name,a.type,a.cateid,b.birthday,b.job,b.sort,b.id,c.catename ' )->alias( 'a' )->join( 'gb_article b', 'a.worknum=b.adminid' )->join( 'gb_cate c', 'c.id=a.cateid' )->where('a.type',"中层干部")->order( 'b.sort,a.cateid,a.worknum' )->where( 'a.worknum', $data[ "information" ] )->paginate( 50 );
				}
			} 
			else {
				$i = $data[ "part" ];
				if ( $data[ "selectway" ] == "name"  && $data["information"]=="") {
					$artres = db( 'admin' )->field( 'a.worknum,a.name,a.type,a.cateid,b.birthday,b.job,b.sort,b.id,c.catename ' )->alias( 'a' )->join( 'gb_article b', 'a.worknum=b.adminid' )->join( 'gb_cate c', 'c.id=a.cateid' )->where('a.type',"中层干部")->order( 'b.sort,a.cateid,a.worknum' )->where( 'a.cateid', $i )->paginate( 50 );
				} 
				elseif ($data[ "selectway" ] == "name") {
					$artres = db( 'admin' )->field( 'a.worknum,a.name,a.type,a.cateid,b.birthday,b.job,b.sort,b.id,c.catename ' )->alias( 'a' )->join( 'gb_article b', 'a.worknum=b.adminid' )->join( 'gb_cate c', 'c.id=a.cateid' )->where('a.type',"中层干部")->order( 'b.sort,a.cateid,a.worknum' )->where( 'a.name', $data[ "information" ] )->where( 'a.cateid', $i )->paginate( 50 );
				}
				elseif ( $data[ "selectway" ] == "worknum" ) {
					$artres = db( 'admin' )->field( 'a.worknum,a.name,a.type,a.cateid,b.birthday,b.job,b.sort,b.id,c.catename ' )->alias( 'a' )->join( 'gb_article b', 'a.worknum=b.adminid' )->join( 'gb_cate c', 'c.id=a.cateid' )->where('a.type',"中层干部")->order( 'b.sort,a.cateid,a.worknum' )->where( 'a.worknum', $data[ "information" ] )->where( 'a.cateid', $i )->paginate( 50 );
				}
			}
			$this->assign( array(
				'artres' => $artres,
  ...
  public function my1(){
	...
	public
	function mysort() {
		$artres = db( 'admin' )->field( 'a.worknum,a.name,a.type,a.cateid,b.id,b.sort,b.job,b.birthday,c.catename ' )->alias( 'a' )->join( 'gb_article b', 'a.worknum=b.adminid' )->join( 'gb_cate c', 'c.id=a.cateid' )->where('a.type',"科级干部")->order( 'b.sort,a.cateid,a.worknum' )->paginate( 250 );
		$department = db( 'cate' )->order( 'id' )->select();
		$this->assign( array(
			'artres' => $artres,
			'department' => $department,
			...
			require(ROOT_PATH . 'vendor/PHPExcel.php');//引入PHP EXCEL类
               ...
               if ($info) {
                    ...
                    foreach ($excelData as $k => $v) {
                        ...
                        $data2['jobtime'] = $v['K'];
                        $data2['ranktime'] = $v['L'];
                        $data2['alljobtime'] = $v['M'];
                        $data2['joinpartytime'] = $v['N'];
                        $data2['edubg'] = $v['O'];
                        //$data2['degree'] = $v['P'];
                        $data2['ptitle'] = $v['J'];
                        //$data2['specialjob'] = $v['Q'];
                    		$data2['zhiji'] = $v['R'];
                        ...
                        /*array(2) {
  ["adminid"] => string(5) "10001"
  ["politics"] => string(12) "中共党员"
}*/
                        //dump($data2);die;
                    		if ( !db('article')->where('adminid', $data2['adminid'])->find()) {
                            db('article')->insert($data2);
                    		} else {
                    			db('article')->where('adminid', $data2['adminid'])->update($data2);
                    		}
                    	}
...
//dump($worknum);die;
    ...
    $app=input('app');
		$worknum = db( 'article' )->field( 'adminid' )->where( array( 'id' => input( 'id' ) ) )->find();
$arts= db("jianli")->where('adminid',$worknum['adminid'])->select();
    if($app)
    {
      $arts1= db("jianli2")->where('adminid',$worknum['adminid'])->select();
    ...
    //dump($id);die;
      ...
      */
      //dump($data);die;
			$save = $article->update(['img' => $data['img'],'id'=>$data['id']]);
      $uid = (int) $data['id'];
      //dump($uid);die;
			if ( $save ) {
				$this->success( '修改干部信息成功！', url( 'edit',['id'=>$uid]) );
			} else {
				$this->error( '修改干部信息失败！' );
			}
			return;
		...
		$cate = new CateModel();
		$cateres = $cate->catetree();//dump($cateres);die;
		$this->assign( array(
			'cateres' => $cateres,
			'arts' => $arts,
      'id' => $id,
